...
class Mongo:
    ...
    def appendDataListToMongo(self, collection, JsonObject, id_field):

        # JsonObject_id = collection.insert_one(JsonObject).inserted_id
        try:
            # auto generated ID
            #collection.update({'_id': ObjectId()}, JsonObject, True)
            collection.update({'_id': JsonObject[id_field]}, JsonObject, True)
        except BaseException as exc:
            print exc
            #self.appendDataListToMongo(collection, JsonObject, id_field)
    ...
    def appendItemToMongo(self,collection, Item_JsonObject, id_field):
        ...
        try:
            ...
            print exc
            self.appendItemToMongo(collection, Item_JsonObject, id_field)
        #return JsonObject_id
        # mongo connection and append logic
    ...
    def getfromMongo(self,collection, query):
        ...
        try:
            ...
            print exc
            self.getfromMongo(collection, query, no_cursor_timeout=True)
